	...
	if(isset($_POST["submitImage"])) {
		if(!empty($_FILES["fileToUpload"]["name"])){
			...
			// if everything is ok, try to upload file
				...
				//sõltuvalt failitüübist loon sobica pildiobjekti
				...
				//leian suuruse muutimise suhtarvu
				...
				//faili salvestamine, j'lle sõltuvalt failitüübist
				if($imageFileType == "jpg" or $imageFileType == "jpeg"){
					if(imagejpeg($myImage, $target_file, 90)){	
						echo "Fail ". basename( $_FILES["fileToUpload"]["name"]). " laeti edukalt üles.";
					}else{
						echo "Tekkis tehniline viga, faili ei laetud üles";
					}
				}
				if($imageFileType == "png"){
